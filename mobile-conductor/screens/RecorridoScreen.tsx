...
export function RecorridoScreen({ onGoToInicio, isFocused }: RecorridoScreenProps) {
  ...
  // Refrescar silenciosamente cada vez que este tab se activa.
  // Usando prop isFocused porque con display:none el useFocusEffect de
  // Expo Router no dispara (no es una pantalla de Stack real).
  useEffect(() => {
    if (isFocused) {
      refreshData(false);
    }
  }, [isFocused]); // refreshData es estable (useCallback), se puede omitir del dep array
  const formatHora = (h: string | null | undefined) => (h ? h.slice(0, 5) : '—');
  ...
  const enProgreso =
    asignacion?.estatus_recorrido?.toLowerCase().includes('progreso') ?? false;

  // ── Sin asignación activa ──────────────────────────────────────────────────
  if (!asignacion || !enProgreso) {
    return (
      <ScrollView
      ...
      >
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyTitle}>No tienes un recorrido activo</Text>
          <Text style={styles.emptyText}>
            Tu recorrido aparecerá aquí{'\n'}cuando inicies una asignación.
          </Text>

          {/* Enlace a Inicio para iniciar el recorrido */}
          <Pressable
            style={({ pressed }) => [styles.goToInicioBtn, pressed && styles.goToInicioBtnPressed]}
  ...
  return (
    ...
    >
      ...
      {/* Acceso al mapa existente — toda la lógica del mapa, movimiento,
          checkpoints y finalización está en mapa/[id].tsx y se reutiliza íntegramente */}
      <TouchableOpacity

...
export const ConductoresEditPage: React.FC = () => {
      ...
      try {
        ...
        setFormData({
          ...
          fecha_expedicion_licencia: data.fecha_expedicion_licencia
            ? String(data.fecha_expedicion_licencia).slice(0, 10)
            : '',
          vigencia_licencia: data.vigencia_licencia
            ? String(data.vigencia_licencia).slice(0, 10)
            : '',
          telefono: data.telefono ? String(data.telefono).replace(/\D/g, '').slice(0, 10) : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.estado === 'Activo' && vigDate < todayDate) {
      return setError(
        'No se puede registrar un conductor como "Activo" con una licencia vencida. Seleccione "Inactivo" o actualice la vigencia.'
      );
    }
        ...
        >
          <form onSubmit={handleSubmit}>
            <div className="form-grid">
              ...
              {/* Estado */}
              <div className="form-field">
                <label className="form-label" style={{ marginBottom: 0 }}>Estado *</label>
                <p style={{ fontSize: '0.8rem', color: 'var(--text)', opacity: 0.8, marginTop: '0.2rem', marginBottom: '0.5rem' }}>
                  Define si el conductor se encuentra activo y disponible para operar rutas.
                </p>
                <select
                  className="form-input"
                  name="estado"
